...
start_time = None  # To measure total runtime
...
def graceful_exit(signal, frame):
    ...
    save_log(log)
    save_skip_list(skip_list)
    print("Logs and skip list saved.")
    if start_time:
        elapsed = datetime.now() - start_time
        print(f"Elapsed time: {elapsed.seconds // 60} minutes and {elapsed.seconds % 60} seconds.")
    sys.exit(0)

# Main function
def main():
    global personal_ip, vpn_process, log, skip_list, start_time
    log = load_log()
    ...
    for config_file in config_files:
        ...
        if vpn_process and not start_time:
            start_time = datetime.now()  # Record start time after first VPN connection
